...
contract High_low
{
    ...
    High_low_token public object= new High_low_token();
    function High_low() public payable
    {
        admin=msg.sender;
        token_contract_address=address(object.token_address);
        this_address=address(this);
    }
    //for High_low_token 
    address public token_contract_address;
    
    High_low_token public tokens;  // creating an instance
    
    function buy_token() public payable returns(bool)
    {
        object.transferFrom(token_contract_address,msg.sender, msg.value/0.1 ether);
        return true;
    }
    
    function add_stake_token(uint256 token_to_stake) public payable returns(bool)
    {
        require(High_low_token(token_contract_address).balanceOf(msg.sender)>=token_to_stake);
        High_low_token(token_contract_address).transferFrom(msg.sender,address(this),token_to_stake);
    ...
    function betting(uint256 _game_id,uint256 _choice,uint256 _bet_tokens) public payable returns(bool) // is_bet_success 
    {
        ...
        require((broker_map[game_id_map_broker[_game_id]].stake_amount>=(90*msg.value)/100)||
        (broker_map[game_id_map_broker[_game_id]].stake_tokens>=(90*_bet_tokens)/100)||
        (broker_map[game_id_map_broker[_game_id]].stake_tokens*100000000000000000>=(90*msg.value)/100) ||
        (broker_map[game_id_map_broker[_game_id]].stake_amount>=(90*_bet_tokens*100000000000000000)/100));
        require(High_low_token(token_contract_address).balanceOf(msg.sender)>=_bet_tokens);
        ...
        gamers_map[_game_id]++;
        if(msg.value!=0 && broker_map[game_id_map_broker[_game_id]].stake_amount>=(90*msg.value)/100)
        {
            ...
            Transfer_amount(msg.sender,this_address,msg.value);
        }
        else 
        {
            High_low_token(token_contract_address).transferFrom(msg.sender,address(this),_bet_tokens);
            broker_map[game_id_map_broker[_game_id]].stake_tokens-=(90*_bet_tokens)/100;
