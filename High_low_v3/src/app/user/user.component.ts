...
export class UserComponent implements OnInit, OnDestroy {
    ...
    et(num)
  {
    ...
    bt(opt,id,chc,amt)
    {
      ...
      {
        this.wcs.game_set_map(id).then(result =>{
    ...
    inc(choice,bid,amount)
    {
      ...
      else
      {
        ...
        this.wcs.game_details(address,id).then(amt => {
          if(amt[1]>0 || amt[2]>0)
          {
        ...
        //  console.log(bet_time);
            ...
            if(choice==0)
         {
          ...
          this.wcs.increase_ether(bid,amount);
         }      
          else if(choice==1)
         {
         
          this.wcs.increase_token(bid,amount)
         }
  ...
  else
  {
          ...
          if(bet_time+120 <=result[2])
           {
             ...
             if(choice==0)
             {
             this.wcs.decrease_ether(bid,amt);
             }
             else if(choice==1)
             {
               this.wcs.decrease_token(bid,amt)
             }
            }          
        else
          {
    ...
    exit(bid)
    {
      ...
      {        
        this.wcs.getAccount().then(address => { 
        this.wcs.game_details(address,bid).then(resu =>{
          if((resu[1] >0 || resu[2] >0) && resu[3]==false)
          {
           ...
           else
           {
             ...
             {
               swal("You are Already Cancelled this Bet");
             }
           }
      })
    })
